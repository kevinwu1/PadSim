...
public class Sim2 {
    ...
    static synchronized void updateOptimal(
            double[] total, int[] optimal, int bo, double score, int i) {
    ...
    static double score(byte[] board) {
        byte[] b = new byte[N];
        for (int i = 0; i < N; i++) {
            b[i] = board[i];
        }
        ...
        int[] combos = { 0 };
        byte[] match = match(b);
        boolean[] FUA = new boolean[] { false };
        double ret = mergeCombo(b, match, FUA, combos);
        score += ret;
        // System.out.println("FUA: " + FUA[0]);
        if (!FUA[0])
            return 0;
        ...
        while (ret != 0) {
            match = match(b);
            ret = mergeCombo(b, match, new boolean[] { false }, combos);
            // System.out.println("Get: " + ret);
            score += ret;
            fall(b, match);
            // print(b);
        ...
        return score * (1.0 + (combos[0] - 1) * .25);
    }
    ...
    static
            double mergeCombo(
                    byte[] b, byte[] match, boolean[] FUA, int[] combos) {
        double score = 0;
    ...
    static class AtomicCounter {
        ...
        public void increment() {
            ...
            if (val % (N / 100) == 0) {
                System.out.println(val / (N / 100) + "% done");
            }
        }
